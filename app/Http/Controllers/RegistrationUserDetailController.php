...
class RegistrationUserDetailController extends Controller
{
    ...
    public function firstpagesave(Request $request)
    {
        ...
        if (empty($request->all())) {
            ...
            // $request['prefer_specializations_id'] = json_encode($request['prefer_specializations_id']);
            // dd($request->all());
    ...
    public function secondpagesave(Request $request)
    {
        ...
        if (empty($request->all())) {
            ...
            $request['graduation_date'] = json_encode($request['graduate_date_yr'] . '-' . $graduation_month);
            // dd($request->except(['_token', 'graduate_date_yr', 'graduate_date_month']));
            // dd($request['qualification_id']);
            // UserDetailInfo::where('user_id', $request['user_id'])->update($request->except(['_token', 'graduate_date_yr', 'graduate_date_month', 'qualification_id']));
            UserDetailInfo::updateOrCreate(['user_id' => $request['user_id']], $request->except(['_token', 'graduate_date_yr', 'graduate_date_month']));

            // for third user detail requirements
            $specializations = Prefer_Specialization::all()->pluck('name', 'id');
    ...
    public function thirdpagesave(Request $request)
    {
        ...
        // dd($request->all());
        $data = ['position_title' => $request['position_title'], 'company_name' => $request['company_name'], 'job_duration_from' => $job_duration_from, 'job_duration_to' => $job_duration_to, 'role_id' => $request['role_id'], 'industries_id' => $request['industries_id'], 'position_level' => $request['position_level'], 'phone_no' => $request['phone_no']];
        if (empty($request->all())) {
            abort(403, 'Sorry this page is not working, Please try again step by step!!!');
        } else {

            UserDetailInfo::updateOrCreate(['user_id' => $request['user_id']], $data);
    ...
    {
        $role = Role::where('prefer_specializations_id', $id)->get();
